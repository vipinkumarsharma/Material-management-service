...
public interface RoleMasterRepository extends JpaRepository<RoleMaster, String> {
    ...
    @Query("SELECT r FROM RoleMaster r WHERE LOWER(r.roleName) LIKE LOWER(CONCAT('%', :name, '%'))")
    Page<RoleMaster> findByRoleNameContainingIgnoreCase(@Param("name") String name, Pageable pageable);
}
